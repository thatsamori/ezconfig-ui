...
function KeyRow({
  ...
  return <>
    <div role="row" className={`config-grid config-row${overridden ? ' has-override' : ''}${muted ? ' feature-muted' : ''}`} title={muted ? 'Feature parameter: stored and sent, but only read while the feature toggle is on.' : undefined}>
      ...
      {groups.map((group, index) => <div role="cell" key={group}><ValueEditor entry={entry} value={rowValues[index]} label={`${database} ${group} ${entry.configKey}`} onChange={value => setValue(database, group, entry.configKey, value)} onReset={() => removeValue(database, group, entry.configKey)} /></div>)}
