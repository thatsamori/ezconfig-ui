/**
 * POST /api/notes/{schema}/edit
 *
 * Edit a note by ID (atomic operation)
 *
 * Request body: { configKey: string, noteId: string, note: string }
 * Response: { success: true, data: NotesData } or { success: false, error: string }
 */

import { NextRequest, NextResponse } from 'next/server';
import { editNote } from '@/lib/notes/service';
...
export async function POST(
  ...
  try {
    ...
    const { configKey, noteId, note } = body;
    ...
    const data = await editNote(schema, configKey, noteId, note);
